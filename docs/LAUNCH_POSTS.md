...
Use these as starting copy for the public launch.
...
1. AI agents are only as good as the context you give them.

2. So we made WANT2VIEW CLI: an open-source connector that turns keyword searches, channel audits, CSV/JSON imports, public catalog samples, and WANT2VIEW projects into source-of-truth packs for Codex and Claude.
...
We just released WANT2VIEW CLI as an open-source entrypoint into WANT2VIEW.

The goal is simple: give Codex, Claude, and terminal agents real content evidence instead of vague prompts.
...
```text
Show HN: Open-source CLI that gives Codex and Claude content evidence packs
```

Post:

```text
I built WANT2VIEW CLI, an open-source connector for Codex, Claude, and terminal agents.

It creates local source-of-truth packs from keyword demos, channel demos, CSV/JSON imports, public catalog samples, and authenticated WANT2VIEW projects.
