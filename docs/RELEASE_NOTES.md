...
- MCP tools include `doctor`, `create_research_plan`, `create_research`, `get_status`, `export_pack`, `get_subtitles`, and `search_telegram`.
- API Access can generate the MCP connection command alongside the fast `account research` CLI pack command.
